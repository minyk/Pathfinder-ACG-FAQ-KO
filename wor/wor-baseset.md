...
# `B` 시나리오가 너무 어렵습니다! 쉽게할 방법이 없을까요(The B scenarios in Wrath are really hard! What if I don't want them to be so hard)?

다음의 두 방법을 원하는 만큼 적용합니다:

* `Tangle Trap` 헨치맨의 파워에 `If defeated, you may immediately attempt to close the location this henchman came from.` 를 추가합니다.
* 다음의 카드들을 세트 지시자를 변경합니다.
 * `B`나 `C` 세트에서 `1`번 세트로 이동:
  * `Arboreal Blight` 베리어
  * `Carrion Golem` 몬스터
  * `B` 세트의 `Demonic Horde` 베리어 1장과 `C` 세트의 모든 `Demonic Horde` 베리어
 * `1`번 세트에서 `B`로 이동:
  * `Accursed Priest` 몬스터
  * `Poisoned Spiked Pit Trap` 베리어
...
## `Tangle Trap`을 물리칠 결우 장소를 닫는 시도를 할 수 없기 때문에 `Elven Entanglement` 시나리오에서 `Vinst`를 얻는 것이 별 도움이 되지 않습니다. 이렇게 되는 게 맞나요(Getting Vinst in The Elven Entanglement doesn't help much, since a Tangle Trap won't allow me to close my location when I defeat it. Is that how it's supposed to work)?

아니오. 더 도움이 되어야 합니다. `Vinst`의 파워를 다음과 같이 변경합니다:
* 어떤 캐릭터가 장소덱에서 핸치맨을 물리쳤을 때 이 카드를 박스로 반환하여 그 캐릭터가 그 장소를 자동으로 닫도록 허용합니다("When any character defeats a henchman from a location deck, return this card to the box to allow that character to automatically close that location").
...
## `Mythic Marshal`의 파워를 사용하기 위해 미씩 차지를 소비하고 그 체크에 성공했을 때 사용한 차지마다 캐릭터를 하나씩 선택할 수 있나요 아니면 사용한 숫자에 관계없이 한번만 선택할 수 있나요(When you expend charges for the Mythic Marshal's power, then succeed at the check, do you choose one other character at your location for each charge you spent, or just one regardless of the number spent)?

얼마나 많은 차지를 사용하는지가 아니라 체크에 성공하는지에만 관심을 가지면 됩니다. 얼마나 많은 차지를 사용하던 단 하나의 다른 캐릭터만 차지를 얻습니다.
...
## `Imrijka`가 16개의 스킬 피트가 있는 것이 맞나요(Should Imrijka have 16 skill feats)?

아니오. `Constitution`에 있는 "+3" 피트를 삭제합니다.
...
## `Lancer` 역할의  `Alain`은 자신의 덱, 손패, 버린 더미에 있는 `Mount` 특성이 있는 카드에 따라 이상할 정도로 많은 탐험을 할수 있습니다. 잘못된 것 아닌가요(Lancer Alain can get a ridiculous amount of explorations going with a card that has the Mount trait in his hand. Or discard pile. Or on his deck. This seems abusable).

잘 못된 것이 맞습니다. `Lancer` 역할의 첫 번째 파워에서 "Recharge a card" 를 "Recharge a weapon or an armor" 로 변경합니다.
...
## `Harsk the Demonslayer`는 첫 번째 탐험에서 `Demon`을 물리치면 공짜 탐험을 받는 파워가 있습니다. 시나리오의 첫 번째 탐험인가요, 턴의 첫 번째 탐험인가요(Harsk the Demonslayer has a power that gives him a free exploration if he defeats a Demon on his first exploration. Is that the first exploration of the scenario, or of the turn)?
...
## `Imrijka`의 `Cold Iron Warden` 역할에는 `Blessings of Pharasma`에 사용되는 파워가 있습니다. 하지만 `Wrath of the Righteous` 게임에는 해당 블레싱이 없습니다(Imrijka's Cold Iron Warden role has a power that works on Blessings of Pharasma, but there are no Blessings of Pharasma in Wrath of the Righteous).

예, 문제가 됩니다. `Cold Iron Warden` 역할에서 `Blessing of Pharasma`를 `blessing that has the Basic trait` 으로 변경합니다.
...
## `Imrijka`의 `Wandering Judge` 역할은 버려진 너무 많은 `Blessing of Ascension` 카드를 블레싱 덱으로 섞어 넣을 수 있습니다(Wandering Judge Imrijka sure can shuffle a lot of discarded Blessing of Ascension cards into the blessings deck).

아니오, 할 수 없습니다. `Wandering Judge` 역할의 해당 파워는 `Basic` 특성이 없는 블레싱을 플레이할 때에만 적용됩니다.
...
## `Wardstone Patrol` 사니라오에서 `Sir Ilivan`을 사용하여 몬스터를 물리치면 `Corrupted Soldier`를 소환하여 인카운터 해야 합니다. 몬스터를 물리치는 체크의 앞서 해야 하나요, 후에 해야 하나요(In The Wardstone Patrol, I'm playing Sir Ilivan on a check to defeat a monster, and have to summon and encounter the Corrupted Soldier. Do I do that before or after I finish with the first monster)?

시작한 것을 끝내고 `Corrupted Soldier`를 인카운터 합니다.
...
## `Abattoir` 장소의 `bane`을 물리치기 위한 난이도는 게임의 캐릭터 수 만큼 올라가나요 아니면 그 장소에 있는 캐릭터 수만 해당되나요(Is the difficulty of checks to defeat banes at the Abattoir increased by the number of characters in the game, or just the number of characters at the Abattoir)?

게임에 있는 캐릭터 수 만큼 올라갑니다.
...
## `Canyon`는 그 장소에 있는 다른 모든 캐릭터들이 같은 몬스터를 인카운트하게 합니다. 그 몬스터들은 소환된 것인가요(When Canyon makes everyone else at the location encounter the monster you're encountering, should their copies be summoned)?
...
## `Ulkreth`은 물리치는 체크를 한 후에 `Knowledge` 체크를 하도록 합니다. 이거 정말 필요한가요(Ulkreth has me attempting a Knowledge check while I am already making a check to defeat. Do you really want that)?

아니오 그 `Knowledge`는 더 일찍 일어나야 합니다. 물리치는 체크에 앞서 `Knowledge` 8 체크에 성공하면 5를 더 할 수 있습니다.
...
## `Arboreal Blight`와 `Corrupted Crusaders`는 소환된 헨치맨의 체크 결과에 따라 물리치거나 물리치지 못하는 것이 결정됩니다. 소환된 헨치맨을 회피하면 어떻게 되나요(Arboreal Blight and Corrupted Crusaders are defeated or undefeated based on the result of an encounter with a summoned henchman. What happens if I evade that summoned henchman)?

그 카드들은 헨치맨을 물리치는지에만 주의하면 됩니다. 소환된 헨치맨을 회피하면 물리친 것이 아니기 떄문에 그 카드들은 물리치지 못한 것이 됩니다.
...
## `Quarterstaff`는 `Strength` 나 `Melee` 주사위만 사용해야 하나요(Should the Quarterstaff use my Strength or Melee die only)?

아니오, 그 스킬을 사용하는 것이 맞습니다.
...
## `Glibness`는 다른 셋에서는 `Elite`인데 여기에서는 `Basic` 입니다. 이게 맞나요(Glibness was Elite in every other set, but it's Basic here. Which should it be)?

예(역자주: 긴 설명은 하단을 참조하세요).
...
## `Portable Altar`를 버리면 얼마나 오래 `Divine`에 1d6을 더해주나요(When I discard Portable Altar, how long does the 1d6 on my Divine checks last)?
...
## `Ring of Climbing`는 장소덱을 섞을 때마다 매번 사용할 수 있나요(Can Ring of Climbing be used every time I shuffle a location deck)?

그렇지는 않습니다. 체크당 혹은 단계당 한 번만 사용할 수 있습니다.
...
## 내 장소에 다른 캐릭터가 없을 때에도 `Riding Horse`를 사용하여 이동할 수 있나요? 내 장소에 다른 캐릭터가 있다면 항상 같이 움직여야 하나요(Can I use Riding Horse to move when there's nobody else at my location? If there are other characters at my location, do I have to move 1 of them)?
