...
## `Brutalized Flesh Golem`은 `Skinstitcher's Manual`을 통해서 손에 들어온 경우에만 표시합니다. 그렇지 않은 경우에는 어떻게 되나요(Brutalized Flesh Golem is only displayed if it gets into your hand via Skinstitcher's Manual. What if it gets into my hand some other way? Also, it seems like its powers should only function while displayed).

예, 수정이 필요하네요.
 
`Skinstitcher's Manual`의 첫 번째 파워의 첫 번째 문장을 다음과 같이 변경합니다: "이 카드를 매장하여 Brutalized Flesh Golem 을 박스에서 꺼내어 손패에 넣습니다." 

`Brutalized Flesh Golem`의 파워를 다음과 같이 변경합니다: "이 카드를 표시할 수 있습니다. 표시된 동안, 이 카드를 재충전하여 같은 장소에 있는 한 캐릭터의 전투 이외의 피해를 0로 줄이거나 이 카드를 버려서 현재 장소를 추가 탐험할 수 있습니다; 만일 카드를 버렸다면, 이 탐험동안 인카운터하는 모든 이로운 카드를 제거하고, 전투 체크에 1d6과 현재의 미씩 충전 수를 더합니다. 표시되어 있는 동안, 자신의 턴의 끝에, 미씩 충전을 가지고 있다면 이 카드를 제거합니다. 
...
## `Demondome` 시나리오에서는 "이 장소 덱들 중 하나가 비어 있으면 그 장소를 폐쇄합니다."가 있습니다. 정확히 하나의 장소 덱만 비어 있을 때 해당 되나요? 그리고 "장소를 닫는다"는 장소 폐쇄를 시도하는 것을 말하나요 아니면 자동으로 장소를 닫으라는 말인가요(Demondome says "When 1 of these location decks is empty, close that location". Does that first part apply only when exactly 1 deck is empty? And does "close that location" mean "attempt to close" or "automatically close")?

시나리오의 어떤 장소든 덱이 비면, 자동으로 닫으라는 것입니다.
...
## `Nocticula's Attention` 시나리오에서 `Blessings of Nocticula`가 표시된 숫자가 캐릭터 수보다 같거나 큰 시점에서 무조건 종료해야 하나요(In Nocticula's Attention, do I have to win at the moment that the number of displayed Blessings of Nocticula is greater than or equal to the number of characters)?

아니오. 그 블레싱을 계속 얻으려면 시나리오를 지속할 수 있습니다-하지만 그 판단에 대한 결과도 당신 책임입니다.
...
## 현재 `Battlebliss`에 있습니다. 여러 번의 체크가 필요한 `Demon` 특성이 있는 몬스터의 첫 번째 체크를 성공(12이상 차이로)한 시점에서 장소 페쐐를 시도할 수 있나요(I'm at Battlebliss. If I succeed (by 12 or more) at my first check to defeat a Demon that has multiple checks to defeat, can I attempt to close the location before I attempt the second check)?

아니오-몬스터를 물리친 후에만 장소 폐쇄를 시도할 수 있습니다.
...
## `Rapture of Rupture`에 의해서 이동된 경우 즉시 다시 돌아 올 수 있나요(If I'm moved by Rapture of Rupture, can't I just move right back)?

그러면 정말 쓸모 없는 파워겠네요!

`Rapture of Rupture` 장소의 "move to a random location" 를 "move to a random location and you may not move during your move step"게 변경합니다.
...
## `Heart of the Fane` 빌런에는 "물리치지 못했을 때, `Grinder`가 열려 있으면 빌런이 도망가지 않습니다."라는 파워가 있습니다. `Grinder`가 임시 폐쇄된 경우는 어떤가요(Heart of the Fane says "If undefeated, and if the location Grinder is open, the villain does not escape." What if the Grinder is temporarily closed)?

일반적으로 임시 폐쇄는 빌런이 그 장소로 도망가는 것을 막는 것외에는 다른 어떤 기능도 없습니다만, `Grinder`가 임시 폐쇄된 경우에도 적용되어야 합니다.
...
## `Nezirrius` 헨치멘은 행동하기 전 `Ranged` 피해를 줍니다. 이건 `Ranged` 전투 피해죠(Nezirrius deals Ranged damage before you act. Should that be Ranged Combat damage)?
...
## `Abyssal Shortcut` 배리어에 있는 "당신"은 배리어를 물리치는 사람인가요 아니면 `Abyssal Shortcut`이 표시되어 있는 장소에 있는 누군가 인가요(On Abyssal Shortcut, who does "you" refer to: the person who defeated the barrier, or whoever's at the location where Abyssal Shortcut is displayed)?
...
## `Climber’s Gloves`는 장소 덱을 섞을 때 마다 한번씩 사용할 수 있나요(Can Climber’s Gloves be used every time I shuffle a location deck)?

정확히는 아닙니다. 아이템은 체크당 혹은 단계당 한번만 사용될 수 있습니다.
