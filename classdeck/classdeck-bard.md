# 바드 클래스 덱(Bard Class Deck)

## `Meliski`가 자신의 장소에 있는 캐릭터의 실패한 체크에서 2개의 주사위를 재굴림하는 파워를 얻은 후에도 원한다면 하나의 주사위만 재굴림할 수 있나요(After Meliski has gained the feat that allows a character at his location to reroll 2 dice after failing a check, can that character reroll just 1 die if they like)?
...
옙. 다음과 같이 두번째 파워를 변경합니다: When you attempt a check to defeat a barrier that has the Skirmish or the Task trait, you may use your Diplomacy skill in place of the listed skill. (▢ Characters gain this power while at your location.)
...
## `Flaming Scimitar +1`은 `Magic` 특성이 있어야 하지 않나요(Should the Flaming Scimitar +1 have the Magic trait)?
...
## 바드 덱의 `Glibness`를 플레이한 했는데 `Arcane` 나 `Divine` 스킬이 없으면 제거해야 하나요(If I play Glibness from the Bard deck, but I don't have the Arcane or Divine skill, do I have to banish it)?
...
## `Swipe` 나 `Princeling` 같은 카드를 `Ivory Dice`를 얻는데 사용할 수 있나요(Can I play a card like Swipe or Princeling to acquire the Ivory Dice)?

아니오, `Ivory Dice`는 획득 체크가 없습니다.
...
## `Fortune-Teller`를 한턴에 두번 사용할 수 있나요(Can I use the Fortune-Teller twice in the same turn)?
...
## `Spiny Shield`와 `Shield of Fire Resistance`는 `Magic` 특성이 있습니다. 이 카드들은 손패를 리셋하는 과정에서 재충전할 수 있어야 하지 않나요(The Spiny Shield and Shield of Fire Resistance have the Magic trait. Shouldn’t they let you recharge them when you reset your hand)?
