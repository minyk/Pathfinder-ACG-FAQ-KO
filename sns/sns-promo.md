...
## `Goblin Weidling`은 클래스 0의 배여서 `Plunder & Peril`을 진행하는 동안 함대에 넣어서 사용할 수 있습니다만 어드벤처 덱 1에 있는 `Riptide Grindylow`를 소환하여 인카운트해야 합니다. 어떻게 해야 하나요(The Goblin Weidling is a Class 0 ship, so it can be part of my fleet during Plunder & Peril. But it requires you to summon and encounter the henchman Riptide Grindylow, which is in Adventure Deck 1. What should I do)?

`Riptide Grindylow`와 인카운트 하십시오.

> Go ahead and fetch the Riptide Grindylow from the Adventure Deck 1 box!

posted Sep 2, 2014 | back to top

## 소환된 `Riptide Grindylow`를 물리치면 `Owlbeartross`를 상대할 때 하나의 체크만 난이도가 4가 올라가요 아니면 둘다 올라가나요? 제발 하나라고 해주세요(If I defeat the summoned Riptide Grindylow, does the difficulty of one check against the Owlbeartross go up by 4, or does it affect both checks? Please say just one).
...
## 프로모 카드는 어떤 것들이 있으며 언제 릴리즈되나요(What are all the promo cards, and how and when will they be released)?
